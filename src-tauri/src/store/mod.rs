...
pub async fn open(app: &tauri::AppHandle) -> Result<SqlitePool, StoreError> {
    ...
    // sqlx expects a URI like sqlite:///absolute/path — the leading / comes from
    // the absolute path itself, so the literal only has two slashes.
    // mode=rwc tells SQLite to create the file if it doesn't exist.
